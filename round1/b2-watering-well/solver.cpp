...
solve1() {
    ...
    for (u32 i = 0; i < N; ++i) {
        ...
        sx = (sx + t.x) % MOD;
        sy = (sy + t.y) % MOD;
        sx2 = (sx2 + (t.x % MOD) * (t.x % MOD)) % MOD;
        sy2 = (sy2 + (t.y % MOD) * (t.y % MOD)) % MOD;
    }
    u64 rx = 0, ry = 0;
    for (u32 k = 0; k < Q; ++k) {
        auto& q = XY[k];
        rx = (rx + sx2) % MOD;
        ry = (ry + sy2) % MOD;
        rx = (rx + u64(N) * ((q.x % MOD) * (q.x % MOD) % MOD) % MOD) % MOD;
        ry = (ry + u64(N) * ((q.y % MOD) * (q.y % MOD) % MOD) % MOD) % MOD;
        rx = (rx + (MOD - (2 * sx * (q.x % MOD)) % MOD)) % MOD;
        ry = (ry + (MOD - (2 * sy * (q.y % MOD)) % MOD)) % MOD;
    }
    u64 ans = (rx + ry) % MOD;
#endif
