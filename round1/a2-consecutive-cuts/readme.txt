Note: The only difference between this chapter and chapter 1 is that here, card
values are not guaranteed to be distinct.

Let's cut to the chase. You have a deck of N face-up cards, each displaying a
not necessarily unique integer between 1 and 10^9. Cutting the deck once
consists of taking a stack of between 1 and N−1 (inclusive) cards from the top
and moving it to the bottom in the same order. For example, for the deck
[5,1,2,4,3] ordered from top to bottom, cutting 2 cards from the top would yield
[2,4,3,5,1]:
...
is a line containing NN space-separated integers, B_1,...,B_N.
